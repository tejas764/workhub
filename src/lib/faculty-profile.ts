...
export const facultyMemberFromRow = (row: FacultyRow, index = 0): FacultyMember => ({
  ...
  departmentId: firstText(row, ["department_id", "dept_id", "department_code", "dept_code"]),
...
export const fallbackFacultyFromUser = (user: SupabaseUser | null): FacultyMember => {
  ...
  return {
    ...
    phone: "Not added",
    designation: "Faculty",
    department: asText(metadata.department) || asText(metadata.department_name) || "Department",
    departmentId: asText(metadata.department_id) || asText(metadata.dept_id) || asText(metadata.department_code) || undefined,
